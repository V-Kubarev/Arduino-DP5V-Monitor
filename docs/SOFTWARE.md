  ...
  Когда bufferIndex достигает значения 29, на следующей итерации (29 + 1) % 30 дает в результате 0. Указатель возвращается в начало массива, и новые
  данные начинают перезаписывать самые старые значения. Именно так и реализуется "скольжение" — старые данные автоматически "забываются".
  ...
  делится на длительность окна.
   ...
   1 float cps = windowPulses / (float)WINDOW_SECONDS;
