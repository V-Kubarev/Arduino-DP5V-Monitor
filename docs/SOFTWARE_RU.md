...
---
  ...
  ## Принцип программного "мертвого времени"

  Для обеспечения высокой точности подсчета импульсов в прошивке реализован механизм программного "мертвого времени". Его главная задача —
  отфильтровывать ложные срабатывания, вызванные особенностями аналогового тракта дозиметра ДП-5В.

  Проблема: "Грязный" сигнал с аудиовыхода

  Аналоговая схема ДП-5В, генерирующая звуковые щелчки для наушников, не была предназначена для формирования чистого цифрового сигнала. В результате
  один реальный распад, зарегистрированный счетчиком Гейгера, может породить на аудиовыходе не один четкий импульс, а более сложный сигнал:

   * Основной импульс, за которым следует серия затухающих колебаний или "звон".
   * "Дребезг" или несколько быстрых, идущих подряд пиков.

  Эти затухающие колебания, которые можно назвать "хвостами" основного импульса, имеют достаточную амплитуду, чтобы несколько раз вызвать срабатывание
  аппаратного прерывания Arduino. Это привело бы к тому, что одно реальное событие было бы посчитано как 2, 3 или даже больше импульсов, что
  искусственно завысило бы показания мощности дозы.

  Решение: Программное "мертвое время" для отсечки "хвостов"

  Чтобы бороться с этим, в коде введена принудительная "пауза" после регистрации каждого валидного импульса.

  > Важно понимать: это не физическое мертвое время самого счетчика Гейгера (которое связано с процессом восстановления инертного газа в трубке). Это
  исключительно программный фильтр, созданный для очистки входного сигнала и отсекания его "хвостов".

  Принцип работы фильтра:
  После того как программа регистрирует первый "фронт" импульса и засчитывает его, она на короткий, строго определенный промежуток времени перестает
  реагировать на любые последующие изменения на пине прерывания.

  Реализация в коде

  Эта логика полностью реализована внутри функции-обработчика прерывания `geigerISR()`.

   1. Определение константы:
      В начале кода задается длительность "периода нечувствительности" в микросекундах.
```
       // 20 000 микросекунд = 20 миллисекунд
       #define DEAD_TIME_US 20000
```
  20 мс — это время, заведомо большее, чем длительность всех "хвостов" и дребезга, но при этом достаточно короткое, чтобы не пропустить следующий
  реальный импульс при умеренных уровнях радиации.

   2. Логика внутри `geigerISR()`:
```
        void geigerISR() {
          // Получаем текущее время в микросекундах
          unsigned long now = micros();
    
          // Проверяем, прошло ли достаточно времени с момента последнего УЧТЕННОГО импульса
          if (now - lastPulseTime > DEAD_TIME_US) {
    
            // --- Этот блок выполняется, только если прошло > 20 мс ---
    
            // 1. Увеличиваем счетчик
            pulseCount++;
   
            // 2. ОБНОВЛЯЕМ ВРЕМЯ ПОСЛЕДНЕГО УЧТЕННОГО ИМПУЛЬСА
            lastPulseTime = now;
   
            // 3. Устанавливаем флаг для звука/света
            beepFlag = true;
          }
          // Если с момента последнего учтенного импульса прошло МЕНЬШЕ 20 мс,
          // то условие if не выполняется, и функция просто завершает работу,
          // полностью ИГНОРИРУЯ текущее срабатывание прерывания.
        }
```
  Таким образом, этот простой, но эффективный фильтр гарантирует, что только первый фронт "грязного" сигнала будет засчитан, а все его последующие
  "хвосты" будут проигнорированы, обеспечивая точность подсчета.
...
## Принцип работы динамического скользящего окна

  Основная цель: Сделать прибор одновременно и стабильным при измерении низкого фона, и отзывчивым при обнаружении источников радиации.

  ### Проблема статического окна

  Классическое скользящее окно (которое было в ранних версиях) имеет фиксированную длину (например, 30 секунд). Это порождает компромисс:

   * Длинное окно (30 сек): Идеально для измерения фона. Оно отлично сглаживает случайные всплески и дает очень стабильные, усредненные показания. Но
     оно очень медленно реагирует на изменения. Если поднести прибор к источнику, пройдет до 30 секунд, прежде чем показания достигнут своего реального
     значения.
   * Короткое окно (например, 5 сек): Идеально для поиска. Оно почти мгновенно реагирует на изменение фона. Но при измерении низкого фона его показания
     будут очень "шумными" и нестабильными, постоянно прыгая.

  ### Решение: Адаптация "на лету"

  Алгоритм динамического окна решает эту проблему, автоматически изменяя длительность усреднения в зависимости от текущей радиационной обстановки. Он
  работает по принципу "лучшее из двух миров".

  > Аналогия: Представьте, что вы едете на машине.
  >    На пустом шоссе (низкий фон):* Вы смотрите далеко вперед, оценивая общую картину (используется длинное окно).
  >    В плотном городском потоке (высокий фон):* Вы фокусируетесь на нескольких метрах прямо перед капотом, чтобы мгновенно реагировать на маневры
  соседей (используется короткое окно).

  Принцип реализации в коде

  Важно понимать, что физический буфер для данных в памяти всегда одного размера (30 ячеек). Динамически меняется лишь то, какую часть этого буфера мы
  используем для расчета среднего значения в данный момент.

  1. Принятие решения

  Каждую секунду программа смотрит на уже усредненное значение cps и принимает решение, какой длины окно использовать для следующего расчета.
```
    // Пороги, заданные в константах
    #define CPS_THRESHOLD_FAST  3.0 // Порог для перехода в быстрый режим
    #define CPS_THRESHOLD_SLOW  1.5 // Порог для возврата в медленный режим
    
    // ...
    
    // Если средний CPS превысил порог, переключаемся в быстрый режим
    if (previous_cps > CPS_THRESHOLD_FAST) {
      currentWindowDuration = FAST_WINDOW_SECONDS; // Становится 5 сек
   }
   // Если средний CPS упал ниже другого порога, возвращаемся в медленный
   else if (previous_cps < CPS_THRESHOLD_SLOW) {
     currentWindowDuration = SLOW_WINDOW_SECONDS; // Становится 30 сек
   }
```
  Гистерезис: Обратите внимание, что пороги для включения и выключения быстрого режима разные (3.0 и 1.5). Это сделано для того, чтобы избежать
  "дребезжания" режимов, когда CPS колеблется вокруг одного значения.

  2. Динамическое суммирование

  После того как `currentWindowDuration` определена (5 или 30), запускается цикл суммирования. Он суммирует не все 30 ячеек, а только последние `N`
  значений, где `N = currentWindowDuration`.
```
...
```
  Этот цикл всегда берет только самые "свежие" данные в количестве, необходимом в данный момент.

  3. Расчет среднего

  Финальное значение CPS рассчитывается делением на текущую (динамическую) длительность окна, а не на фиксированную.
```
...
```
  ...
  Таким образом, ваш прибор постоянно адаптируется к окружающей обстановке: он обеспечивает максимальную точность и стабильность при фоновых измерениях
  и максимальную скорость реакции, когда это действительно необходимо — при поиске источников и в зонах с повышенным уровнем радиации.

---

  ## Принцип поправки на физическое мертвое время трубки

  Основная цель алгоритма — повышение точности измерений путем математической компенсации физического ограничения счетчика Гейгера-Мюллера, известного
  как "мертвое время". Это позволяет получать достоверные данные о скорости счета в широком диапазоне, особенно при высоких уровнях ионизирующего
  излучения.

  Проблема:

  После регистрации ионизирующей частицы и последующего газового разряда, любому счетчику Гейгера-Мюллера требуется конечный промежуток времени для
  возвращения в исходное состояние, в котором он снова способен зарегистрировать следующую частицу. Этот период восстановления, называемый физическим
  мертвым временем (`τ`, тау), является фундаментальной характеристикой детектора.

   * При низких интенсивностях излучения, когда интервал между частицами велик, этот эффект пренебрежимо мал.
   * При высоких интенсивностях, когда частицы поступают с высокой частотой, значительная их часть попадает в детектор, пока тот находится в "мертвом"
     состоянии. Эти события не регистрируются, что приводит к систематической нелинейной погрешности — занижению реальной скорости счета.

  Аналогия: Представьте высокоскоростную фотокамеру со вспышкой, которая фотографирует молнии в грозу. После каждой вспышки конденсатору требуется
  время на перезарядку (τ). Если молнии сверкают редко, камера успевает зафиксировать каждую. Но во время сильной грозы, множество вспышек молний
  произойдут, пока вспышка камеры перезаряжается, и эти события не будут запечатлены.

  Решение:

  Решение заключается в математической коррекции измеренной скорости счета (N_изм) для получения оценки истинной скорости счета (N_ист), какой она была
  бы при отсутствии мертвого времени. Для непарализуемой модели счетчика (где событие, попавшее в мертвое время, просто игнорируется), используется
  следующая формула:

  > N_ист = N_изм / (1 - N_изм * τ)

  Логика формулы:
   1. Выражение `N_изм * τ` вычисляет общую долю времени в пределах одной секунды, в течение которой детектор был неработоспособен ("мертв"),
      обрабатывая предыдущие события.
   2. Выражение `1 - (N_изм * τ)` определяет долю времени, в течение которой детектор был работоспособен ("жив") и готов к регистрации новых частиц.
   3. Разделив измеренное количество импульсов (N_изм) на долю "живого" времени, мы нормализуем результат, получая оценку того, сколько импульсов было
      бы зарегистрировано, если бы детектор был работоспособен 100% времени.

  Реализация в коде

  Алгоритм реализован в основном цикле программы, после этапа усреднения скорости счета по динамическому окну.

  1. Определение параметра `τ`

  В начале кода определяется константа, соответствующая паспортной характеристике используемого счетчика, выраженная в секундах. Точность этой
  константы напрямую влияет на точность компенсации.
```
...
```
    ...
    // Для отображения на экране используется уже скорректированное значение
    float cps = true_cps;
```
  Таким образом, прошивка сначала получает усредненное, но заниженное значение measured_cps, затем математически корректирует его до true_cps и уже
  это, более точное значение, использует для всех последующих вычислений и отображения.

---
 
  ## Принцип пересчета CPS в мощность дозы (µSv/h)

  Одним из центральных алгоритмов дозиметра является преобразование "сырых" данных от счетчика Гейгера в общепринятые единицы измерения, отражающие
  биологическое воздействие радиации.

  Задача: От "кликов" к биологическому воздействию

  Сам по себе счетчик Гейгера — это просто "счетчик событий". Он регистрирует, сколько раз через него прошла ионизирующая частица, и выдает
  соответствующее количество электрических импульсов. Измеряемая им величина CPS (Counts Per Second) — это просто "количество импульсов в секунду".

  Однако эта величина не говорит о степени биологического воздействия на человека. Для этого используется системная единица Зиверт (и ее производные,
  например, микрозиверт в час, µSv/h), которая оценивает эквивалентную дозу излучения.

  Задача алгоритма — преобразовать физическую величину (CPS) в биологически значимую (µSv/h).

  Проблема: Неуниверсальность пересчета

  Не существует единой универсальной формулы для пересчета CPS в µSv/h. Этот пересчет является самой большой сложностью и источником погрешности в
  любом дозиметре. Коэффициент преобразования сильно зависит от двух главных факторов:

   * Тип и чувствительность счетчика Гейгера: Большой счетчик при том же уровне радиации выдаст больше импульсов, чем маленький. Разные
      газы-наполнители и материалы катода также влияют на чувствительность.
   * Энергия регистрируемого излучения: Счетчик имеет разную эффективность регистрации для гамма-квантов разной энергии. Например, он может хорошо
      "видеть" излучение от Цезия-137, но хуже "видеть" излучение от Кобальта-60.

  Представьте, что вы пытаетесь оценить ущерб для крыши от осадков. Просто считать "удары в секунду" (аналог CPS) недостаточно. Нужно знать, что именно
  падает: капли дождя или градины размером с кулак (аналог разной энергии излучения).

  Реализация в коде: Линейный калибровочный коэффициент

  Для упрощения в данном проекте, как и в большинстве бытовых дозиметров, используется линейный калибровочный коэффициент. Этот подход предполагает,
  что мы измеряем излучение с заранее известным или "усредненным" энергетическим спектром (например, естественный радиационный фон).

  В коде это реализовано очень просто:
```
   // Коэффициент, определяющий, сколько "мкЗв/ч" содержится в одном "CPS"
...
```
  Эта формула предполагает, что каждый зарегистрированный "средний" импульс в секунду (CPS) вносит одинаковый, заранее определенный вклад в итоговую
  мощность дозы.

  Важность калибровки

  Число 0.34 в коде — это не случайное значение, взятое "с потолка". Это и есть калибровочный коэффициент, полученный экспериментальным путем.

  Процесс калибровки, описанный в `README.md`, заключается в следующем:
   * Берутся два прибора: наш самодельный дозиметр и эталонный, профессиональный дозиметр (в данном случае ДКГ-03Д).
   * Оба прибора помещаются в одинаковые условия (например, измеряют естественный фон).
   * Записываются их показания: у нашего прибора в CPS, а у эталонного — в µSv/h.
   * Коэффициент вычисляется по простой формуле:
      > Коэффициент = (Показанияэталона, µSv/h) / (Показаниянашего_прибора, CPS)

  Именно поэтому для достижения максимальной точности каждый собранный прибор в идеале должен быть откалиброван индивидуально, так как даже два
  одинаковых счетчика Гейгера могут иметь немного разную чувствительность.

---

 ## Принцип расчета эквивалента Б-8 (CPM)

  На основном экране прибора, помимо мощности дозы, отображается значение, помеченное как B8. Это значение представляет собой не дозиметрическую, а
  радиометрическую величину, предназначенную для конкретной практической задачи — проверки работоспособности прибора по контрольному источнику.

  Назначение: От дозы к количеству частиц

  Важно понимать фундаментальное различие между двумя основными величинами, которые показывает прибор:

   * Мощность дозы (µSv/h): Это дозиметрическая величина, которая пытается оценить биологическое воздействие ионизирующего излучения (преимущественно
     гамма) на человека. Она зависит от калибровочного коэффициента и энергии излучения.
   * Скорость счета (CPS/CPM): Это радиометрическая величина, которая просто показывает, сколько событий (распадов) регистрирует счетчик Гейгера за
     единицу времени. Она не пытается оценить биологический эффект.

  Контрольный источник Б-8, встроенный в ДП-5В, является бета-источником. Переводить показания от чистого бета-источника в зиверты (которые
  стандартизированы для гамма- и рентгеновского излучения) — физически некорректно. Правильный способ оценить его активность — это посчитать, сколько
  бета-частиц он испускает.

  Поэтому значение B8 на экране — это и есть скорость счета в импульсах в минуту (Counts Per Minute, CPM).

  Принцип реализации в коде

  1. Исходные данные

  В качестве основы берется уже усредненное по скользящему окну значение cps — среднее количество импульсов в секунду.

  2. Формула пересчета

  Чтобы перевести скорость счета из "в секунду" в "в минуту", нужно просто умножить значение на 60.

  > CPM = CPS * 60

  3. Код

  В прошивке это реализовано с помощью отдельной константы для ясности:
```
   // Коэффициент пересчета "в секунду" -> "в минуту"
   const float CPS_TO_B8 = 60.0;
   
   // ...
   
   // Расчет эквивалента Б-8 (то есть, CPM)
   unsigned long b8Value = (unsigned long)(cps * CPS_TO_B8);
```
   
  Результат преобразуется в целочисленный тип unsigned long для удобства отображения на дисплее.

  Практическое применение

  При проведении проверки работоспособности прибора с использованием встроенного в ДП-5В источника Б-8, пользователь должен ориентироваться именно на
  значение `B8:` (т.е. CPM) на экране.

  Сравнивая полученное значение CPM с паспортным или ранее измеренным значением для данного источника (если оно известно), можно оценить исправность и
  текущую чувствительность счетчика Гейгера. Это гораздо более точный метод проверки, чем попытка оценить бета-источник в зивертах.
