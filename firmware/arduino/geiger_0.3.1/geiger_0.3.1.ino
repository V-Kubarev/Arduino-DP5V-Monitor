...
#define DEAD_TIME_US 3000


// --- Adaptive software dead time (anti-bounce) ---
// --- Адаптивное программное мёртвое время (антидребезг) ---

// Current adaptive dead time in microseconds
// Текущее адаптивное мёртвое время (мкс)
volatile uint32_t dynamicDeadTime_us = DEAD_TIME_US;
...
void geigerISR() {
  ...
  if (now - lastPulseTime > dynamicDeadTime_us) {
    pulseCount++;
    lastPulseTime = now;
    beepFlag = true;
  }
...
/*
============================
Adaptive dead time update
Обновление адаптивного мёртвого времени
============================
*/

void updateAdaptiveDeadTime(float cps) {

  uint32_t newDeadTime;

  // Select dead time based on averaged CPS
  // Выбор мёртвого времени по усреднённому CPS
  if (cps < 50.0) {
    newDeadTime = 3000;   // Background / фон
  }
  else if (cps < 150.0) {
    newDeadTime = 2500;   // Elevated background / повышенный фон
  }
  else if (cps <= 300.0) {
    newDeadTime = 2000;   // High range limit / верх диапазона
  }
  else {
    // Out of reliable range — do NOT reduce further
    // Вне достоверного диапазона — дальше не уменьшаем
    newDeadTime = 2000;
  }

  // Apply only if changed
  // Применяем только если изменилось
  if (newDeadTime != dynamicDeadTime_us) {
    dynamicDeadTime_us = newDeadTime;
  }
}
  ...
  */
  ...
  if (nowMillis - lastSecondMillis >= 1000) {
    ...
    */
    ...
    // Update adaptive dead time based on averaged CPS
    // Обновляем адаптивный антидребезг по усреднённому CPS
    updateAdaptiveDeadTime(cps);
    ...
    */
    ...
    if (displayMode == DISPLAY_MAIN) {
      ...
      lcd.setCursor(15, 1);
      if (dynamicDeadTime_us == 3000) lcd.print("S");
      else if (dynamicDeadTime_us == 2500) lcd.print("M");
      else lcd.print("F");
